...
def genConfig(seq_path, set_type):
    ...
    if set_type == 'VisEvent':
        vis_img_list   = sorted([seq_path + '/vis_imgs/' + p for p in os.listdir(seq_path) if os.path.splitext(p)[1] == '.bmp'])
        event_img_list = sorted([seq_path + '/event_imgs/' + p for p in os.listdir(seq_path) if os.path.splitext(p)[1] == '.bmp'])
        gt = np.loadtxt(seq_path + '/groundtruth.txt', delimiter=',')

    return vis_img_list, gt
...
def train_mdnet():
    ...
    ## set image directory
    if pretrain_opts['set_type'] == 'VisEvent': 
        img_home = '/home/wangxiao/Documents/rgb_event_tracking_benchmark/rtmdnet-visEvent-tracking-baseline/visEvent_dataset/train_subset/'
        data_path = '/home/wangxiao/Documents/rgb_event_tracking_benchmark/rtmdnet-visEvent-tracking-baseline/visEvent_dataset/VisEvent_train_subset.pkl'
...
if __name__ == "__main__":
    ...
    parser.add_argument("-set_type", default = 'VisEvent' )
    parser.add_argument("-padding_ratio", default = 5., type =float)
    parser.add_argument("-model_path", default ="./models/rt_mdnet_vis.pth", help = "model path")
